...
Last audit: cycle 23. Source: `dist/*.html` + `src/styles/layout.min.css` vs `mirror/mr`.
...
| `/downloads` | 33/33 | 22/22 | (selector) | 100 |
...
Leftover (not in formula): downloads `downloadCenterInputWrapper`; language modal; footer `.socialMedia` omitted on purpose.
