...
Last audit: cycle 09. Source: `dist/*.html` + `src/styles/layout.min.css` vs `mirror/mr`. Motion from `dist/js/mr/*`. Live preview: curl 200 (SSO off).
...
Leftover (not in formula): `/typo3conf/ext/powermail/` images 404; footer `.socialMedia` omitted; no language modal.
