...
const impulses = [
  { title: "Longdong ±800 kV", body: "CHVT on an HVDC line, May 2025. High-voltage work, not a solar slogan.", fill: solarFill },
  { title: "4–6 weeks", body: "Standard lead time for catalogue types. Say so on the first screen, not in a footnote.", fill: factoryFill },
...
txt(desk, {
  text: "Our range. Your transformer.",
...
const worlds = [
  ...
  { title: "Motor drive", sub: "CMA7 · SHM-D. HWV already includes one.", fill: cma7Fill, x: 740, y: 1716 },
];
worlds.forEach((w) => {
...
txt(desk, {
  text: "Start with these four.",
  x: 56, y: 2106, w: 640, h: 32, size: 28,
  family: uiSB.family, style: uiSB.style,
});

const latest = [
  { name: "CV2", line: "Vacuum selector. 350 A and 600 A only.", fill: cv2Fill },
  { name: "CM2", line: "Vacuum diverter + selector. Same envelope as CM.", fill: cm2Fill },
  { name: "SHZV", line: "Vacuum OLTC. 400 / 600 / 1000 A.", fill: shzvFill },
  { name: "HWV", line: "Horizontal vacuum. Drive already on the list.", fill: hwvFill },
];
latest.forEach((p, i) => {
  const n = frame(desk, {
...
[
  ...
  ["ISO 17025", "type-test laboratory"],
...
txt(help, {
  text: "We are here for you. Wherever you are.",
...
[
  ["CV2", "Vacuum selector. 350 / 600 A."],
  ["CM2", "Vacuum CM envelope."],
  ["SHZV", "400 / 600 / 1000 A."],
  ["HWV", "Horizontal. Drive included."],
].forEach((row, i) => {
  const n = frame(phone, {
...
txt(mnum, {
  text: "4–6 weeks · ISO 17025 · 002270.SZ",
...
txt(prod, {
  text: "Products  /  Vacuum OLTC  /  CV2",
  x: 56, y: 128, w: 600, h: 16, size: 12,
  family: ui.family, style: ui.style, color: C.muted,
});

const phero = frame(prod, { name: "photo", x: 720, y: 168, w: 664, h: 520, fill: C.stage });
phero.fills = darkPhoto(cv2Fill);
txt(prod, {
  text: "CV2",
  x: 56, y: 176, w: 620, h: 56, size: 56,
  family: uiSB.family, style: uiSB.style,
});
txt(prod, {
  text: "Vacuum selector-switch OLTC. The vacuum version of CV, for primarily-delta transformers up to 132 kV.",
  x: 56, y: 248, w: 620, h: 72, size: 18,
  family: ui.family, style: ui.style, color: C.ink, lh: 26,
});
txt(prod, {
  text: "On-load: it changes taps while the transformer is energised. Arc in a vacuum interrupter, not in oil.",
  x: 56, y: 332, w: 620, h: 48, size: 15,
  family: ui.family, style: ui.style, color: C.muted, lh: 22,
});
btn(prod, { label: "Technical data PDF", x: 56, y: 400, w: 196, fontName: uiM });
const pghost = frame(prod, {
  name: "btn-Leaflet", x: 264, y: 400, w: 128, h: 44, fill: C.white,
  stroke: { color: C.ink, opacity: 0.2 },
});
txt(pghost, {
  text: "Leaflet", x: 0, y: 13, w: 128, h: 18, size: 14,
  family: uiM.family, style: uiM.style, color: C.ink, align: "CENTER",
});

const specs = [
  ["Switching", "Vacuum selector switch. Mechanical backup contacts. No grade letter."],
  ["Current", "III 350 A and 600 A only. There is no CV2-500."],
...
txt(prod, {
  text: "Also look at",
  x: 56, y: 1032, w: 400, h: 24, size: 20,
  family: uiSB.family, style: uiSB.style,
});
[
  ["CV", "Oil ancestor of CV2", cmFill],
  ["CM2", "Vacuum diverter + selector", cm2Fill],
  ["SHZV", "Higher current vacuum OLTC", shzvFill],
].forEach((row, i) => {
  const n = frame(prod, {
    name: row[0], x: 56 + i * 444, y: 1072, w: 428, h: 200, fill: C.white,
    stroke: { color: C.ink, opacity: 0.12 },
  });
  ...
  txt(n, {
    text: row[1], x: 180, y: 88, w: 220, h: 40, size: 13,
    family: ui.family, style: ui.style, color: C.muted, lh: 18,
  });
});
...
txt(mprod, {
  text: "Vacuum selector-switch OLTC. 350 A and 600 A only. No CV2-500.",
...
return {
  ...
  children: figma.currentPage.children.map((c) => c.name),
